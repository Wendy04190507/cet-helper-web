...
export default function Listening() {
  ...
  if (exercises.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
        ...
        <h3 className="text-lg font-semibold mb-2">暂无听力练习</h3>
  ...
  return (
    <div className="px-4 pt-6 pb-4">
      <h2 className="text-2xl font-semibold mb-1">听力训练</h2>
      <p className="text-sm text-text-secondary mb-6">精听 + 听写 + 主动回忆</p>
      <div className="space-y-3">
        {exercises.map((item, idx) => (
          <Link key={idx} to={`/listening/${idx}`} className="card card-hover flex items-center gap-4 p-4 bg-white rounded-2xl border border-divider/50">
